...
function Library.new(title)
	...
	local titleLabel = Create("TextLabel", {
		...
		BorderColor3 = COLOR_BLACK,
		Text = self.title,
		Position = UDim2.new(0.01403, 0, 0, 0),
		Parent = topbar,
	})
	...
	local tabFrame = Create("Frame", {
		BorderSizePixel = 0,
		BackgroundColor3 = COLOR_WHITE,
		Size = UDim2.new(0, 426, 0, 18),
		Position = UDim2.new(0.14552, 0, 0, 0),
		BorderColor3 = COLOR_BLACK,
		BackgroundTransparency = 1,
		Parent = topbar,
	})
	...
	self._outsideClickConn = Services.User_Input.InputBegan:Connect(function(input, gameProcessed)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			if self.main.Visible then
				local mainAbs = self.main.AbsolutePosition
				local mainSize = self.main.AbsoluteSize
				if mousePos.X >= mainAbs.X and mousePos.X <= mainAbs.X + mainSize.X and mousePos.Y >= mainAbs.Y and mousePos.Y <= mainAbs.Y + mainSize.Y then
					return
				end
			end

			if self.colorPickerPopup and self.colorPickerPopup.Visible then
				local absPos = self.colorPickerPopup.AbsolutePosition
				local absSize = self.colorPickerPopup.AbsoluteSize
			...
			if self.keyMenuPopup and self.keyMenuPopup.Visible then
				local absPos = self.keyMenuPopup.AbsolutePosition
				local absSize = self.keyMenuPopup.AbsoluteSize
				if mousePos.X < absPos.X or mousePos.X > absPos.X + absSize.X or mousePos.Y < absPos.Y or mousePos.Y > absPos.Y + absSize.Y then
			...
			for _, ctxData in ipairs(self._openContextPopups) do
				if ctxData.open.value and ctxData.popup.Visible then
					local absPos = ctxData.popup.AbsolutePosition
					local absSize = ctxData.popup.AbsoluteSize
					if mousePos.X < absPos.X or mousePos.X > absPos.X + absSize.X or mousePos.Y < absPos.Y or mousePos.Y > absPos.Y + absSize.Y then
						ctxData.open.value = false
						tween(ctxData.popup, {GroupTransparency = 1}, TWEEN_INFO_FAST):Play()
						task.delay(0.15, function()
							ctxData.popup.Visible = false
						end)
					end
				end
			end
...
function Library:_createKeyMenuPopup(parent)
	...
	holdBtn.MouseButton1Click:Connect(function()
		...
		holdBtn.TextColor3 = COLOR_ACCENT
		toggleBtn.TextColor3 = Color3.fromRGB(155, 155, 155)
		alwaysBtn.TextColor3 = Color3.fromRGB(155, 155, 155)
		startListening()
	...
	toggleBtn.MouseButton1Click:Connect(function()
		...
		holdBtn.TextColor3 = Color3.fromRGB(155, 155, 155)
	...
	alwaysBtn.MouseButton1Click:Connect(function()
		...
		holdBtn.TextColor3 = Color3.fromRGB(155, 155, 155)
		toggleBtn.TextColor3 = Color3.fromRGB(155, 155, 155)
		alwaysBtn.TextColor3 = COLOR_ACCENT
		startListening()
		tween(km, {GroupTransparency = 1}, TWEEN_INFO_FAST):Play()
		task.delay(0.15, function()
			km.Visible = false
...
function Library:CreateToggle(section, config)
	...
	function toggleData:CreateColorPicker(config)
		...
		cpButton.MouseButton1Click:Connect(function()
			...
			lib.colorPickerCallback = function(c)
				...
				cpButton.TextColor3 = c
				Library.flags[cpFlag] = c
				cpCallback(c)
			end
			lib.colorPickerFlag = cpFlag
			lib.colorPickerRGB = cpDefault
	...
	function toggleData:AddContextMenu()
		...
		ctxPopup.Size = UDim2.new(0, 235, 0, 10)
...
function Library:Notify(text, isWarning)
	local barColor = isWarning and Color3.fromRGB(255, 200, 0) or COLOR_ACCENT
	...
	local notifyText = Create("TextLabel", {
		...
		BackgroundTransparency = 1,
		Size = UDim2.new(0, 87, 0, 20), BorderColor3 = COLOR_BLACK,
		Text = text or "",
		Position = UDim2.new(0.0394, 0, 0.09091, 0),
		Parent = notifyFrame,
	})
	...
	notifyFrame.Position = UDim2.new(0, -210, notifyFrame.Position.Y.Scale, notifyFrame.Position.Y.Offset)
	tween(notifyFrame, {Position = UDim2.new(0, 4, notifyFrame.Position.Y.Scale, notifyFrame.Position.Y.Offset)}, TWEEN_INFO_SMOOTH):Play()
	tween(timerBar, {Size = UDim2.new(0, 0, 0, 2)}, TweenInfo.new(3, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)):Play()
	...
	task.delay(3, function()
		tween(notifyFrame, {Position = UDim2.new(0, -210, notifyFrame.Position.Y.Scale, notifyFrame.Position.Y.Offset)}, TWEEN_INFO_SMOOTH):Play()
...
function Library:CreateMultiTab(tab, name)
	if not tab._multiTabs then
		...
		local tabButtons = Create("Frame", {
			BorderSizePixel = 0, BackgroundColor3 = COLOR_WHITE,
			Size = UDim2.new(0, 471, 0, 18),
			Position = UDim2.new(0.014, 1, 0, 6),
			BorderColor3 = COLOR_BLACK, BackgroundTransparency = 1,
	...
	local mtCanvas = Create("CanvasGroup", {
		...
		Position = UDim2.new(-0.0004, 0, 0.08142, 0),
		BorderColor3 = COLOR_BLACK,
...
function Library:ShowKeybinds(visible)
